...
// Secret, unguessable path for the private request monitor. Not linked anywhere.
// (If you ever change this, update the Cloudflare Access application's path too.)
const DASH_PATH: &str = "/ops-c1a9f7e3b6d2";
...
async fn fetch(req: Request, env: Env, ctx: Context) -> Result<Response> {
    ...
    let resp = Router::new()
        ...
        .get_async(DASH_PATH, |req, ctx| async move {
...
fn should_log(p: &str) -> bool {
    !(p == "/api/hit" || p == "/api/ping" || p.starts_with(DASH_PATH))
}
...
fn dashboard_page(d: &Dash, email: &str) -> String {
    ...
    layout("", "request monitor", "Private request monitor.", DASH_PATH, "", &inner)
}
